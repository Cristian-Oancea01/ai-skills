...
description: Heavyweight DAG orchestration for enterprise-scale projects with explicit specialist roles, state ownership, and remediation cycles
license: MIT
...
Use this skill for the largest and most structured work:
- enterprise-grade projects
- large features with multiple dependencies
- work where explicit role boundaries matter
- tasks where auditability and deterministic stage ownership are important
- complex programs likely to need repeated remediation cycles

Do not use this skill for everyday large tasks. Use `agent-orchestration` for the medium tier and `small-task-orchestration` for the small tier.
...
- Keep sections compact — rewrite or merge stale entries rather than appending indefinitely
...
### Commit discipline
- Commit SKILL.md changes in a dedicated commit after the fix or discovery is confirmed working
- Message format: `<skill-name>: <brief description of what was learned or fixed>`
